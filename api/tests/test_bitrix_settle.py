...
from sync_bitrix_invoice_payments import settle  # noqa: E402
...
class ОстатокПоСчёту(unittest.TestCase):
    ...
    def test_переплата_ровно_на_допуске_ещё_шум(self):
        self.assertEqual(settle(D("100.00"), D("101.00")), D("0.00"))
    ...
    def test_настоящая_переплата_уходит_человеку(self):
        ...
        # самая мелкая из настоящих - 36 единиц сверх счёта
        self.assertIsNone(settle(D("2160.00"), D("2196.00")))

    def test_переплата_чуть_больше_допуска_уходит_человеку(self):
        self.assertIsNone(settle(D("100.00"), D("101.02")))
